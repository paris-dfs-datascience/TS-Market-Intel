...
Run standalone (outside main.py) for testing:
    python -m tools.backfill_results 2026-05-14
    python -m tools.backfill_results all
"""
...
from market_intel.engine import (
    HTTP_USER_AGENT,
    MODEL,
    NO_URL_PLACEHOLDER,
    SEMAPHORE_SIZE,
...
if TYPE_CHECKING:
    from market_intel.storage import Sink
    ...
    async with sem:
        ...
        for signal_type, hits in result.get("signals", {}).items():
            ...
            for hit in hits:
                ...
                if not url or url == NO_URL_PLACEHOLDER:
                    continue
        ...
        for (signal_type, hit), alive in zip(all_hits, statuses):
            ...
            if new_url and await _is_url_alive(new_url, http_client):
                ...
                hit["source_url"] = NO_URL_PLACEHOLDER
                stats["dead_nulled"] += 1
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print("Usage: python -m tools.backfill_results <YYYY-MM-DD|all>")
        sys.exit(1)
